...
Ninguna necesita el cartucho: se hacen sobre src/golf.asm y
src/golf.notes. Vigilan que el listado no se degrade sin que nadie se
...
ASM = os.path.join(RAIZ, "src", "golf.asm")
NOTES = os.path.join(RAIZ, "src", "golf.notes")
TRACE = os.path.join(RAIZ, "work", "golf.trace.json")
...
OTROS_JUEGOS = ("Pitfall", "Temptations", "Stardust", "Ale Hop", "Colt 36",
                ...
                "Demonia", "Nemesis", "Sky Jaguar", "Tennis")
...
class TestWeb(unittest.TestCase):
    ...
    def test_las_paginas_no_inventan_direcciones(self):
        ...
        with open(os.path.join(RAIZ, "golf.rom"), "rb") as f:
            rom = f.read()
        t = Tracer(rom, ORG)
